...
class PlayState extends MusicBeatState
{
	...
	override public function create()
	{
		...
		if (practiceMode || cpuControlled || opponentChart)
		...
		if (!inEditor)
		{
			...
			if (stageData.hide_girlfriend == false)
			{
				...
				if (gfGroupFile != null && gfGroupFile.characters != null && gfGroupFile.characters.length > 0)
				{
					...
					{
						addCharacter(gfGroupFile.characters[i].name, i, false, false, gfGroup,
							gfGroupFile.characters[i].position[0] + gfGroupFile.position[0], gfGroupFile.characters[i].position[1] + gfGroupFile.position[1],
							0.95, 0.95);
			...
			if (dadGroupFile != null && dadGroupFile.characters != null && dadGroupFile.characters.length > 0)
			{
				...
				{
					addCharacter(dadGroupFile.characters[i].name, i, false, opponentChart, dadGroup,
						dadGroupFile.characters[i].position[0] + dadGroupFile.position[0], dadGroupFile.characters[i].position[1] + dadGroupFile.position[1]);
				}
			}
			else
			{
			...
			if (bfGroupFile != null && bfGroupFile.characters != null && bfGroupFile.characters.length > 0)
			{
				...
				{
					addCharacter(bfGroupFile.characters[i].name, i, true, !opponentChart, boyfriendGroup,
						bfGroupFile.characters[i].position[0] + bfGroupFile.position[0], bfGroupFile.characters[i].position[1] + bfGroupFile.position[1]);
				}
			}
			else
			{
		...
		if (!inEditor)
		{
			...
			healthBar = new FlxBar(healthBarBG.x + 4, healthBarBG.y + 4, (opponentChart ? LEFT_TO_RIGHT : RIGHT_TO_LEFT), Std.int(healthBarBG.width - 8),
				Std.int(healthBarBG.height - 8), this, 'shownHealth', 0, 2);
		...
		if (!inEditor && isStoryMode && !seenCutscene)
		{
			switch (curSong)
			{
				case "monster":
					...
					for (gf in gfGroup)
					{
						if (gf.animation.exists('scared'))
							gf.playAnim('scared', true);
					}
					for (boyfriend in boyfriendGroup)
					{
						if (boyfriend.animation.exists('scared'))
	...
	function addCharacter(name:String, index:Int = 0, flipped:Bool = false, isPlayer:Bool = false, ?group:FlxTypedSpriteGroup<Character>, xOffset:Float = 0,
			yOffset:Float = 0, scrollX:Float = 1, scrollY:Float = 1):Character
	{
						...
						for (group in chars)
						{
							for (char in group)
							{
								if (char.danceEveryNumBeats > 0
									&& tmr.loopsLeft % Math.round(char.danceEveryNumBeats) == 0
	...
	function startSong():Void
	{
		...
		if (!inEditor)
		{
			...
			if (showSongText)
			{
				FlxTween.tween(songTxt, {alpha: 1}, 0.5, {
	...
	private function generateSong():Void
	{
		...
		precacheList.set(getNoteFile('noteSplashes'), 'image');
		for (note in unspawnNotes)
		{
			...
			if (!note.noteSplashDisabled
				&& note.noteSplashTexture != null
				&& note.noteSplashTexture.length > 0
				&& !precacheList.exists(getNoteFile(note.noteSplashTexture)))
			{
	...
	function eventPushed(event:EventNote)
	{
		switch (event.event)
		{
			...
			case 'Change Stage':
				...
				if (stageData.directory != null && stageData.directory.length > 0)
					Paths.setCurrentLevel(stageData.directory);
				var dummyStage = new Stage(stage, this);
				var grps = [
					dummyStage.background,
					dummyStage.overGF,
					dummyStage.overDad,
					dummyStage.foreground
				];
				for (grp in grps)
				{
					for (obj in grp)
		...
		if (ClientPrefs.keybindReminders && !cpuControlled)
		{
			...
			for (i in 0...playerKeys)
			{
				...
				for (j in 0...keybinds.length)
				{
					...
					keybindTweenGroup.set(txt, FlxTween.tween(txt, {alpha: 0}, 0.5, {
						startDelay: (offset + 3) * playbackRate,
	...
	override public function update(elapsed:Float)
	{
		...
		if (!inEditor)
		{
			...
			if (!inCutscene)
			{
				if (!startingSong
					...
					&& (boyfriend.animation.curAnim.name.startsWith('idle') || boyfriend.animation.curAnim.name.startsWith('dance')))
				{
		...
		if (unspawnNotes[0] != null)
		{
			...
			while (unspawnNotes.length > 0 && unspawnNotes[0].strumTime - Conductor.songPosition < time)
			{
				...
				callOnScripts('onSpawnNote', [
					strumLineNotes.members[strumID].allNotes.members.indexOf(dunceNote),
					dunceNote.noteData,
					dunceNote.noteType,
			...
			if (!inCutscene)
			{
				if (!inEditor && controls.gamepadsAdded.length < 1)
				{
					for (char in playerChar)
					{
						...
						if (!keyPressed
							&& char.holdTimer > Conductor.normalizedStepCrochet * 0.0011 * char.singDuration
							&& char.animation.curAnim != null
							&& char.animation.curAnim.name.startsWith('sing')
		...
		if (!ClientPrefs.underlayFull)
		{
			if (playerStrums.length > 0)
				underlayPlayer.x = playerStrums.receptors.members[0].x;
		}
		...
		for (i in 0...strumLineNotes.length)
		{
			strumLineNotes.members[i].allNotes.forEachAlive(function(daNote:Note)
			{
				...
				if (strumLine != null && strumLine.receptors.members[daNote.noteData] != null)
				{
					...
					if (strumDirection == 90
						&& daStrum.sustainReduce
					...
					if (daNote.exists
						...
						&& !daNote.isOnScreen(daNote.camera)
	...
	{
		if (!cpuControlled && ((skipHealthCheck && instakillOnMiss) || health <= 0))
			practiceFailed = true;
		else
			return false;
		if (!practiceMode && !isDead)
		{
			...
			if (ret != FunkinLua.Function_Stop)
			{
				...
				#if DISCORD_ALLOWED
				DiscordClient.changePresence(detailsGameOverText, '$curSongDisplayName ($storyDifficultyText)', iconP2.char);
				#end
				isDead = true;
				return true;
			}
	...
	public function moveCamera(isDad:Bool)
	{
		if (isDad)
		{
			camFollow.set(CoolUtil.getCamFollowCharacter(dad).x, CoolUtil.getCamFollowCharacter(dad).y);
		...
		{
			camFollow.set(CoolUtil.getCamFollowCharacter(boyfriend).x, CoolUtil.getCamFollowCharacter(boyfriend).y);
			...
			{
				cameraTwn = FlxTween.tween(FlxG.camera, {zoom: 1}, (Conductor.normalizedCrochet / 1000), {
	...
	function tweenCamIn()
	{
		...
		{
			cameraTwn = FlxTween.tween(FlxG.camera, {zoom: 1.3}, (Conductor.normalizedCrochet / 1000), {
				ease: FlxEase.elasticInOut,
				onComplete: function(twn:FlxTween)
				{
					cameraTwn = null;
	...
	private function cachePopUpScore()
	{
		...
		precacheList.set(getUIFile('combo'), 'image');

		for (i in 0...10)
			precacheList.set(getUIFile('num$i'), 'image');
	}
	...
	private function popUpScore(note:Note = null):Void
	{
		...
		{ // force sick rating if on botplay
		...
		if (!cpuControlled && startedCountdown && !paused)
		{
			...
			if (key > -1 && (FlxG.keys.checkStatus(eventKey, JUST_PRESSED) || controls.gamepadsAdded.length > 0))
			{
				if ((inEditor || !playerChar.members[0].stunned) && generatedMusic && !endingSong)
				{
					...
					{
						if (daNote.noteData == key && daNote.canBeHit && daNote.mustPress && !daNote.tooLate && !daNote.wasGoodHit && !daNote.blockHit)
						{
					...
					if (sortedNotesList.length > 0)
					{
						for (epicNote in sortedNotesList)
						{
							for (doubleNote in pressNotes)
							{
								...
								{
									doubleNote.kill();
									playerStrums.removeNote(doubleNote);
									doubleNote.destroy();
								}
								else
									notesStopped = true;
	...
	{ // You didn't hit the key and let it go offscreen, also used by Hurt Notes
		...
		if (!inEditor)
		{
			...
			for (char in characters)
			{
				if (char < charGroup.members.length
					&& charGroup.members[char] != null
					&& !daNote.noMissAnimation
					&& charGroup.members[char].hasMissAnimations
					&& !charGroup.members[char].skipSing)
				{
					var animToPlay:String = '${playerStrums.animations[daNote.noteData]}miss${daNote.animSuffix}';
					if (charGroup.members[char].animation.exists(animToPlay))
					{
		...
		callOnScripts('noteMiss', [
			playerStrums.members.indexOf(daNote),
			daNote.noteData,
			daNote.noteType,
			daNote.isSustainNote,
			daNote.characters
	...
	function noteMissPress(direction:Int = 1):Void // You pressed a key when there was no notes to press for this key
	{
		...
		if (inEditor || !playerChar.members[0].stunned)
		{
			...
			if (!inEditor)
			{
				...
				for (char in playerChar)
				{
					if (char.hasMissAnimations && char.animation.exists(animToPlay) && !char.skipSing)
					{
	...
	function goodNoteHit(note:Note, strumGroup:StrumLine, isPlayer:Bool = true, ?keys:Array<FlxKey>):Void
	{
		...
		if (!note.mustPress || !note.wasGoodHit)
		{
			...
			if (!inEditor && characters.length < 1)
			{
			...
			if (isPlayer)
			{
				...
				if (note.hitCausesMiss)
				{
					...
					if (!inEditor && note.playAnim != null && note.playAnim.length > 0 && !note.noMissAnimation)
					{
						for (i in characters)
						{
							if (i < charGroup.members.length
								&& charGroup.members[i] != null
								&& charGroup.members[i].animation.exists(note.playAnim))
							{
					...
					if (i < charGroup.members.length && charGroup.members[i] != null && !charGroup.members[i].skipSing)
					{
						...
						else
						{
							...
							if (charGroup.members[i].animation.exists(animToPlay))
							{
								...
								if (keys != null)
									charGroup.members[i].keysPressed = keys;
							}
						}
					}
				}
			...
			callOnScripts('onNoteHit', [
				strumGroup.allNotes.members.indexOf(note),
				note.noteData,
				note.noteType,
				note.isSustainNote,
				note.characters,
				strumLineNotes.members.indexOf(strumGroup),
				isPlayer
			]);
		...
		if (!inEditor)
		{
			...
			if (curBeat >= 0)
			{
				...
				for (group in chars)
				{
					...
					{
						if (char.danceEveryNumBeats > 0
							&& curNumeratorBeat % (Math.round(char.danceEveryNumBeats * (Conductor.timeSignature[1] / 4))) == 0
								&& !char.stunned
								&& char.animation.curAnim != null
								&& !char.animation.curAnim.name.startsWith("sing"))
						{
	...
	{
		if (strumLineNotes.members[strumID].keys == keys && !keysInit)
			return;
	...
	{
		if (inEditor || chartingMode || cpuControlled)
			return null;
